...
- Determine ADR requirement deterministically using two paths: ADR required for decision-level architecture/operating-model changes; no new ADR artifact required for implementation-only changes under an existing accepted decision when linkage metadata is complete.
...
- When ADR is not required, issue REQUEST CHANGES if required existing-ADR linkage metadata is missing (ADR-Required: No, Primary-ADR, ADR-Status-At-Merge: Accepted, ADR-Implementation-Rationale).
